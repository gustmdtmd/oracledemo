...
VALUES('value1', value2); 문자나 날짜이면  ' ', 숫자면 필요없음
-------------------------------------------------*/
...
--salary 컬럼의 값을 8000d으로 업데이트 하세요.
...
--emp01 테이블에서 first_name컬럼의 값이 'candy'일 때 삭제하세요.
DELETE FROM emp01
...
SELECT * FROM user_tables; --만들어 놓은 테이블 확인
SELECT table_name FROM user_tables;
SELECT table_name FROM user_tables
WHERE table_name = 'EMP01';

COMMIT; -- commit 을 해줘야 db에서 나온다.
...
CREATE TABLE emp02(    /* create는 DDL에 포함되어있고 사용시 자동 commit이 된다.*/ 
    emp_id number primary key,    /* NOT NULL + UNIQUE */
    first_name varchar2(50),    /*varchar2 = 가변, char = 고정*/
    salary number
);
