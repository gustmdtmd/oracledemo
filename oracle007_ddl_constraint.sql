...
--297.8589 : 소수점 이하는 반올림해서 삽임함
INSERT INTO student(name, age, avg, hire)
VALUES('홍길동', 30, 297.8589, sysdate);

SELECT * FROM student;

-- 5297.8 : 소수점 2자리로 무존건 계산을 한다. 즉, 정수는 3자리까지만 허용한다.
-- ORA-01438: value larger than specified precision allowed for this column
INSERT INTO student(name, age, avg, hire)
...
--데이터가 삽입되어 있는 상태에서 실제값보다 작게 줄일 수는 없다.
...
SELECT * FROM user_constraints
WHERE constraint_name LIKE '%EMP06%'; --제약조건명은 대문자로 저장되기때문에 꼭 대문자로 입력할것
...
CREATE TABLE emp08(
    ...
    /* 컬럼 레벨 선정시에는 fk는 foreign key(deptno)를 써주지않는다.*/
    locno number(2),
...
CREATE OR REPLACE TRIGGER dept_tri   -- dept_tri 가 있으면 원래 있던건 지우고 만들고 없으면 새로 만들라
AFTER UPDATE ON dept01 FOR EACH ROW  --업데이트 후 dept01의 각 항을 수행하라
BEGIN
    UPDATE emp08
    SET deptno=50
    WHERE deptno=20;
